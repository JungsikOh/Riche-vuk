...
void CullingRenderPass::CreatePipeline() {
  ...
  CreateViewingBoundingBoxPipeline();
}
...
void CullingRenderPass::CraeteGrahpicsPipeline() {
  ...
  std::array<VkDescriptorSetLayout, 6> setLayouts = {
      ...
      g_DescriptorManager.GetVkDescriptorSetLayout("Transform0"),
...
void CullingRenderPass::CreateOcclusionCullingComputePipeline() {
  ...
  std::array<VkDescriptorSetLayout, 4> setLayouts = {g_DescriptorManager.GetVkDescriptorSetLayout("SamplerList_ALL"),
                                                     g_DescriptorManager.GetVkDescriptorSetLayout("ViewProjection_ALL"),
...
void CullingRenderPass::CreateViewingBoundingBoxPipeline() {
...
void CullingRenderPass::CreateBindlessResources() {
  ...
  transformBuilder.BindBuffer(0, &transformUBOInfo, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_ALL, true);

  g_DescriptorManager.AddDescriptorSet(&transformBuilder, "Transform_ALL", true);
...
void CullingRenderPass::CreateBatchDescriptorSets() {
  void* pData = nullptr;
  int batchIndex = 0;

  for (MiniBatch& batch : g_BatchManager.m_miniBatchList) {
    vkMapMemory(m_pDevice, batch.m_transformListBufferMemory, 0, batch.m_transformBufferSize, 0, &pData);
    memcpy(pData, g_BatchManager.m_trasformList.data(), batch.m_transformBufferSize);
    vkUnmapMemory(m_pDevice, batch.m_transformListBufferMemory);

    VkDescriptorBufferInfo transformSSBOInfo = {};
    transformSSBOInfo.buffer = batch.m_transformListBuffer;  // Buffer to get data from
    transformSSBOInfo.offset = 0;                            // Position of start of data
    transformSSBOInfo.range = batch.m_transformBufferSize;   // size of data

    VkUtils::DescriptorBuilder batchBuilder = VkUtils::DescriptorBuilder::Begin(&g_DescriptorLayoutCache, &g_DescriptorAllocator);
    batchBuilder.BindBuffer(0, &transformSSBOInfo, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_ALL, false);
    g_DescriptorManager.AddDescriptorSet(&batchBuilder, "Transform" + std::to_string(batchIndex), false);

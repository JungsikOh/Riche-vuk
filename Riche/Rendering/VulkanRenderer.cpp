...
void VulkanRenderer::Initialize(GLFWwindow* newWindow, Camera* newCamera)
{
	window = newWindow;
	camera = newCamera;
...
void VulkanRenderer::Draw()
{
	...
	submitInfo.pWaitSemaphores = &m_pCullingRenderPass.get()->GetSemaphore();			// List of semaphores to wait on, Command buffer가 실행하기전 대기해야하는 semaphores
...
void VulkanRenderer::FillOffScreenCommands(uint32_t currentImage)
{
	...
	vkCmdBindDescriptorSets(m_SwapchainCommandBuffers[currentImage], VK_PIPELINE_BIND_POINT_GRAPHICS, m_OffScreenPipelineLayout,
		0, 1, &g_DescriptorManager.GetVkDescriptorSet("OffScreenInput"), 0, nullptr);
